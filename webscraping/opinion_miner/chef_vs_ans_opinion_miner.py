...
def lang_pref(df):
    ...
    for index, row in df.iterrows():
        if df.at[index, 'class'] in ans or df.at[index, 'class'] in chef:
             rel_comments = rel_comments + " " + df.at[index, "comments"] 
            ...
            if len(re.findall("ruby", df.at[index,"comments"], re.IGNORECASE))!=0:
                 ruby_cnt_neg+=1
    return python_cnt_pos, ruby_cnt_pos, python_cnt_neg, ruby_cnt_neg, rel_comments
...
if __name__ == '__main__':
    ...
    name_counter(df)
    python_cnt_pos, ruby_cnt_pos, python_cnt_neg, ruby_cnt_neg, rel_comment = lang_pref(df)
    ...
    wordcloud = WordCloud(background_color="white").generate(rel_comment)
